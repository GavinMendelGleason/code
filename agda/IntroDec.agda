...
record Sorting : Set₂ where
  ...
  contra : ∀ x y → ¬ (x ≤A y) → ¬ (y ≤A x) → ⊥
  contra x y p q with x ≤Adec y
  contra x y p q | yes p₁ = p p₁
  contra x y p q | no ¬p with y ≤Adec x
  contra x y p q | no ¬p | yes p₁ = q p₁
  contra x y p q | no ¬p | no ¬p₁ = {!!}
  ...
  safeOnTopNeg {x} {a} ε l₂ nle f s | yes p = Sorted∷ s (Forall∷ f p)
  safeOnTopNeg {x} {a} ε l₂ nle f s | no ¬p = {!!}
