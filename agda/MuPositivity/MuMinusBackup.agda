...
{-
    gfpWFMonotone : (x a : Atom) → x ≢ a → (φ : Shape) → (i : Interpretation) → (F : Subjects) → (Acc _⊂_ F) →
      ∀ X Y → X ⊆ Y → ⟦ φ ⟧ (i [ a ≔ X ]) ⊆ ⟦ φ ⟧ (i [ a ≔ Y ])
      -------------------------------------------------------------
      Σ[ S' ∈ Subjects ] ⟦ φ ⟧ ((i [ x ≔ S' ]) [ a ≔ X ]) ⊆ ⟦ φ ⟧ ((i [ x ≔ S' ]) [ a ≔ Y ])
    gfpWFMonotone x a np φ i F ac X Y sub with ⟦ φ ⟧ ((i [ x ≔ F ]) [ a ≔ X ]
-}
...
-}
    ...
    Monotone i X Y a (ν x s) nin (Nu pos xinp xnin) sub =
      let res = Monotone (i [ x ≔ gfp x s i ]) X Y a s nin pos sub
      in {!!}
