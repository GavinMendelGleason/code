...
$\AD{⊥}$ are in $\AD{Set}$ and therefore in the domain of the family
of unary predicates $\AF{Pred}$. This is essentially the {\em
subobject classifier} approach taken in category theory
\cite{1992Book}.
...
we hope Agda can infer it automatically and unambiguously from
context. If Agda cannot determine the argument automatically then it
...
classify which elements belong, and which do not. We give the
$\AF{IsZero}$ predicate as an example of a unary predicate over the
natural numbers $\AD{ℕ}$. It is a computable function which supplies
the set $\AD{⊤}$ precisely when the number is zero, and supplies
